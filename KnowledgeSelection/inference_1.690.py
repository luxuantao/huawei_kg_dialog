...
class KnowledgeSelection():
    ...
    def get_intent(self, entities, query, context):
        ...
        with torch.no_grad():
            ...
            if len(entity_logits) >= self.args.cand_size:
                for _ in range(self.args.cand_size):
                    index = np.argmax(entity_logits)
                    pred_entities.append((es_entities[index], entity_logits[index]))
                    entity_logits = list(entity_logits)
        ...
        cands = []
        for pred_entity, entity_logit in pred_entities:
            ...
            with torch.no_grad():
                ...
                for attr in attrs:
                    ...
                    outputs = self.intent_model(input_ids=sample['input_ids'].to(self.intent_model.device), 
                                ...
                                token_type_ids=sample['token_type_ids'].to(self.intent_model.device))
                    logit = outputs[1][0][0][1].item()
                    intent_logits.append(logit)
            index = np.argmax(np.array(intent_logits))
            intent_logit = intent_logits[index]
            pred_intent = attrs[index]
        
            cands.append((pred_entity, pred_intent, entity_logit, intent_logit))
        
        cands.sort(key=lambda x: x[2] + x[3], reverse=True)
        pred_entity = cands[0][0]
        pred_intent = cands[0][1]
            
        return pred_intent, pred_entity, error, cands[0][2] + cands[0][3]
...
def main():
    ...
    parser.add_argument('--cand_size', type=int, default=2)  # TODO
