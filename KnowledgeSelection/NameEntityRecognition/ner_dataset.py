...
class NERDataset(Dataset):
    ...
    def collate_fn(self, batch):
        ...
        token_ids = torch.tensor(self.pad_ids(token_ids, self.tokenizer.pad_token_id)).long()
        attention_mask = torch.tensor(self.pad_ids(attention_mask, self.tokenizer.pad_token_id)).float()
        label_ids = torch.tensor(self.pad_ids(label_ids, self.tag2idx["O"])).long()
